...
(function() {
  ...
  function formatDec(num, base, opts) {
    ...
    if (opts.precision) {
      ...
      num = workingNum < 10 ? (Math[ROUND](workingNum * 10) / 10) : workingNum.toFixed(opts.decimal);
    }
    num = num.toString();
  ...
  var SEPTILION = 1000000000000000000000000;
  ...
  function approximateNumber(num, opts) {
    ...
    if (num < thousandsBreak) {
      ...
      numString = formatDec(num, MILLION, opts) + ' million';
    } else if (num < TRILLION) {
      numString = formatDec(num,  BILLION, opts) + ' billion';
    } else if (num < QUADRILLION) {
      numString = formatDec(num,  TRILLION, opts) + ' trillion';
    } else if (num < QUINTILLION) {
      numString = formatDec(num,  QUADRILLION, opts) + ' quadrillion';
    } else if (num < SEXTILLION) {
      numString = formatDec(num,  QUINTILLION, opts) + ' quintillion';
    } else if (num < SEPTILION) {
      numString = formatDec(num,  SEXTILLION, opts) + ' sextillion';
    } else if (num < OCTILLION) {
      numString = formatDec(num,  SEPTILION, opts) + ' septillion';
    }else{
      numString = formatDec(num,  OCTILLION, opts) + ' octillion';
    }
